...
import (
	...
	"github.com/cloudflare/terraform-provider-cloudflare/cmd/migrate-v2/struct_transform/v4_models"
)
...
func (p *HCLParser) ParseDNSRecord(block *hclwrite.Block) (*v4_models.DNSRecordV4Model, error) {
	...
	for name, attr := range body.Attributes() {
		if err := p.parseAttribute(name, attr, model); err != nil {
			...
			slog.Warn("Failed to parse attribute", "attribute", name, "error", err)
		}
	}
	...
	for _, nestedBlock := range body.Blocks() {
		if nestedBlock.Type() == "data" {
			...
			if err != nil {
				...
				slog.Warn("Failed to parse data block", "error", err)
				continue
			}
